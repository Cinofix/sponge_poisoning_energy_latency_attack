...
class _Kettle():
    ...
    - poisonloader
    - poison_ids
    - trainset/poisonset/sourceset/patched_sourceset
    ...
    def __init__(self, args, batch_size, augmentations, mixing_method=dict(type=None, strength=0.0),
        ...
        self.poisonloader = torch.utils.data.DataLoader(self.poisonset, batch_size=validated_batch_size,
                                                        shuffle=self.args.pshuffle, drop_last=False,
                                                        num_workers=num_workers,
                                                        pin_memory=PIN_MEMORY)

        if self.args.pretrain_dataset is not None:
            self.pretrainloader = torch.utils.data.DataLoader(self.pretrain_trainset,
                                                              batch_size=min(self.batch_size, len(self.trainset)),
                                                              shuffle=True, drop_last=False, num_workers=num_workers,
                                                              pin_memory=PIN_MEMORY)
            self.prevalidloader = torch.utils.data.DataLoader(self.pretrain_validset,
                                                              batch_size=min(self.batch_size, len(self.validset)),
                                                              shuffle=False, drop_last=False, num_workers=num_workers,
                                                              pin_memory=PIN_MEMORY)
        ...
        if args.ablation < 1.0:
            ...
            self.partialloader = torch.utils.data.DataLoader(self.partialset,
    ...
    def print_status(self):
        ...
        if self.poison_setup["poison_class"] is not None:
            print(
                f'--Poison images drawn from class {class_names[self.poison_setup["poison_class"]]}  (id. {self.poison_setup["poison_class"]}) .')
        else:
    ...
    def prepare_diff_data_augmentations(self, normalize=True):
        ...
        # Final_Training augmentations are handled separately as they possibly have to be backpropagated
        if self.augmentations is not None or self.args.paugment:
            ...
            elif 'GTSRB' in self.args.dataset:
                params = dict(source_size=32, target_size=32, shift=8, fliplr=True)
            elif 'Celeb' in self.args.dataset:
                params = dict(source_size=64, target_size=64, shift=8, fliplr=True)
    ...
    def initialize_poison(self, initializer=None):
        ...
        print("Initializer: ", initializer, " ", init.data.min(), init.data.max())
    ...
    def export_poison(self, poison_delta, path=None, mode='full'):
        ...
        def _torch_to_PIL(image_tensor):
            ...
            image_torch_uint8 = image_denormalized.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu',
                                                                                                         torch.uint8)
        ...
        if mode == 'packed':
            ...
            for enum, (source, _, idx) in enumerate(self.sourceset):
                ...
                _save_image(source, target_class, idx, location=os.path.join(path, 'sources', names[target_class]),
                            train=False)
            ...
            with open(path + f'kette_{self.args.dataset}{self.args.net[0]}.pkl', 'wb') as file:
            ...
            for lookup, key in enumerate(
                    self.poison_lookup.keys()):  # This is a different order than we usually do for compatibility with the benchmark
    ...
    def select_poisons(self, victim, selection):
        ...
        if selection == 'max_gradient':
            ...
            labels = torch.tensor([data[1] for data in self.poisonset]).to(device=self.setup['device'],
                                                                           dtype=torch.long)
