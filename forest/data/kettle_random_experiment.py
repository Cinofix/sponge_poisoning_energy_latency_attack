...
class KettleRandom(_Kettle):
    ...
    def random_construction(self):
        ...
        self.patched_sourceset = None  # will be set with backdoor (in case)
    ...
    def _parse_threats_randomly(self):
        ...
        elif self.args.threatmodel == 'random':
            source_class = None
    ...
    def _choose_poisons_randomly(self):
        ...
        if self.poison_setup['source_class'] is not None:
            ...
            for index in range(len(self.trainset)):  # we actually iterate this way not to iterate over the images
                ...
                if source == self.poison_setup['source_class']:
                    class_ids.append(idx)
            self.source_train_num = self.args.sources
            ...
            if len(total_ids) < self.source_train_num:
                ...
                self.source_train_num = len(total_ids)
            else:
                print('Surrogate validset size is {}'.format(self.source_train_num))

            self.source_train_ids = np.random.choice(total_ids, size=self.source_train_num, replace=False)
